...
namespace :db do
  ...
  task :populate_domainify_min => :environment do
    ...
    def add_pages
      ...
      Page.create(:title => 'Testimonials', :body => 'Testimonials', :meta_title => 'Testimonials', :show_in_footer => true, :can_delete => false, :parent_id => home.id, :account_id => $CURRENT_ACCOUNT.id, :main_column_id => @default_layout.id) if @cms_config['features']['testimonials']
